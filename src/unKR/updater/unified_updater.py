"""从 update 库移植的 UnifiedConfidenceUpdater，已适配 UnKRModelAdapter。

关键适配说明：

* ``model.entity_emb`` / ``model.relation_emb`` / ``model.mlp_mean`` /
  ``model.mlp_var`` / ``model.predict()`` / ``model.forward()`` 均由
  :class:`~unKR.model.UKGModel.updater_adapter.UnKRModelAdapter` 统一提供。
* 去除了原始 ``HeteroscedasticBaseModel`` 中 GNN 层的冻结逻辑，因为 unKR
  模型不含 GNN 层。
* 保留所有 4 个核心阶段及所有辅助方法，消融实验控制通过 ``ablation_mode``
  参数传入。
"""
...
class UnifiedConfidenceUpdater:
    ...
    该更新器通过结合拓扑标签传播、课程微调、K-hop 因果影响评估和局部的贝叶斯
    表征精炼等流水线，来稳健地处理新事实（以及知识库外 / OOKB 实体）的插入，
    并极力避免灾难性遗忘。

    Args:
        model: :class:`~unKR.model.UKGModel.updater_adapter.UnKRModelAdapter`
            实例。
        dataset: :class:`~unKR.data.IncrementalDataset.UKGDataset` 实例。
        lr: 更新器学习率，默认 0.001。
        gamma: 因果影响计算中的全局衰减因子，默认 0.8。
        device: 运行设备，默认 ``"cpu"``。
        args: argparse 配置命名空间，可为 ``None``（此时使用内置默认值）。
        ablation_mode: 消融实验模式，``"full"`` 或逗号分隔的消融项列表。

            支持的消融项：

            * ``"wo_geom_init"`` — 移除几何中心初始化。
            * ``"wo_topo_prop"`` — 移除拓扑传播，退化为全局均值。
            * ``"wo_anchor"`` — 移除立锚阶段。
            * ``"wo_ent_reg"`` — 移除实体空间防漂移正则。
            * ``"wo_causal"`` — 移除局部因果隔离，默认全部牵连。
            * ``"wo_bayes"`` — 移除贝叶斯滤波，退化为算术均值。
            * ``"wo_func_anchor"`` — 切断功能性输出锚定权重。
    """

    def __init__(self, model, dataset, lr=0.001, gamma=0.8, device="cpu",
                 args=None, ablation_mode="full"):
        self.model = model
    ...
    def step(self, new_facts_batch):
        ...
        Args:
            new_facts_batch (list): 包含 ``(h, r, t)`` 或 ``(h, r, t, conf)``
                元组的列表。``conf`` 为 ``None`` 时表示无标注事实。

        Returns:
            tuple: ``(new_mu, change_mean, change_max, affected_count)``

            * ``new_mu`` — 新事实的最终预测置信度张量。
            * ``change_mean`` — 旧事实置信度平均变化量。
            * ``change_max`` — 旧事实置信度最大变化量。
            * ``affected_count`` — 受影响旧事实数量。
        """
        ...
        with torch.no_grad():
            ...
            mu_without, _ = self.model.predict(
                old_z[base_edge_idx[0]], base_edge_type, old_z[base_edge_idx[1]]
            )
        ...
        S_tau, mu_with, sigma_sq_old = self._compute_causal_influence(
            base_edge_idx, base_edge_type, base_edge_conf, mu_without,
            h_idx, r_idx, t_idx, new_mu, new_sigma_sq, has_label_mask
        )
        ...
        real_affected_count = self._local_representation_refinement(
            base_edge_idx, base_edge_type, c_new, old_raw_emb,
            S_tau, h_idx, r_idx, t_idx, new_mu
        )
        ...
        for i in range(len(h_np)):
            ...
            self.dataset.belief_state[fact_tuple] = (
                float(known_conf_np[i]) if has_label_np[i] else float(new_mu_np[i])
            )
    ...
    def _make_selective_grad_hook(update_mask):
        """创建选择性梯度钩子，仅允许 ``update_mask`` 为 True 的实体接收梯度。"""
        def hook(grad):
    ...
    def _topological_confidence_propagation(self, h_idx, r_idx, t_idx,
                                            base_edge_idx, base_edge_type,
                                            base_edge_conf):
        """利用 1-hop 图拓扑随机游走，将先验置信度传播给无标签的新事实。"""
        N = h_idx.shape[0]
        propagated_conf = torch.zeros(N, dtype=torch.float, device=self.device)
        global_mean = (base_edge_conf.mean()
                       if base_edge_conf.numel() > 0
    ...
    def _propagate_then_finetune(self, h_idx, r_idx, t_idx,
        ...
        with torch.no_grad():
            ...
            if "wo_topo_prop" in self.ablation_mode:
                ...
                propagated_conf = torch.full_like(propagated_conf, global_mean.item()
        ...
        trainable_params = (
            [self.model.entity_emb.weight]
            + [p for p in self.model.mlp_mean.parameters() if p.requires_grad]
        )
        ...
        if has_label_mask.any() and anchor_steps > 0:
            for step_a in range(anchor_steps):
                ...
                loss_ent_reg = lambda_ent_reg * torch.mean(
                    (curr_new_ent_emb - initial_new_ent_emb) ** 2
                )

                loss_mlp_anchor = torch.tensor(0.0, device=self.device)
                for name, param in self.model.mlp_mean.named_parameters():
                    if param.requires_grad and name in old_mlp_params:
                        loss_mlp_anchor = loss_mlp_anchor + torch.mean(
                            (param - old_mlp_params[name]) ** 2
                        )
    ...
    def _get_k_hop_subgraph(self, center_nodes, edge_index, num_hops=2):
        """返回以 ``center_nodes`` 为中心的 K-hop 子图边掩码。"""
    ...
    def _compute_causal_influence(self, base_edge_idx, base_edge_type, base_edge_conf,
                                  mu_without, h_idx, r_idx, t_idx,
                                  new_mu, new_sigma_sq, has_label_mask=None):
        """阶段 2：基于 K-hop 子图的因果推断评估。

        Returns:
            tuple: ``(S_tau_global, mu_with_global, sigma_sq_old_global)``
        """
        self.model.eval()
        ...
        with torch.no_grad():
            curr_z_sub = self.model(
                combined_sub_edge_idx, combined_sub_edge_type, combined_sub_edge_conf
            )

            sub_mu_with, sub_sigma_sq_old = self.model.predict(
                curr_z_sub[sub_base_edge_idx[0]],
                sub_base_edge_type,
                curr_z_sub[sub_base_edge_idx[1]],
            )
    ...
    def _bayesian_belief_filtering(self, base_edge_idx, base_edge_type,
                                   mu_with, sigma_sq_old, S_tau):
        """阶段 3：贝叶斯信念滤波，根据因果影响强度更新旧事实置信度。

        Returns:
            tuple: ``(c_new, c_old)``，更新后和更新前的置信度张量。
        """
        epsilon = getattr(self.args, 'epsilon', 1e-4) if self.args else 1e-4
        ...
        for i in range(len(h_list)):
            fact_tuple = (int(h_list[i]), int(r_list[i]), int(t_list[i]))
    ...
    def _local_representation_refinement(self, base_edge_idx, base_edge_type,
                                         c_new, old_z, S_tau,
                                         h_idx, r_idx, t_idx, new_mu):
        """阶段 4：局部表征精炼，对受影响的实体做联合微调。

        Returns:
            int: 受影响旧事实数量。
        """
        threshold = getattr(self.args, 'influence_threshold', 0.01) if self.args else 0.01
        lambda_reg = min(
            getattr(self.args, 'lambda_reg', 0.001) if self.args else 0.001, 0.01
        )
        func_anchor_ratio = (
            getattr(self.args, 'func_anchor_ratio', 0.9) if self.args else 0.9
        )
        alpha = 1.0
        steps = getattr(self.args, 'refine_steps', 3) if self.args else 3
        base_num_ent = self.dataset.base_num_ent
        ...
        old_ents_for_reg = (
            affected_old_ents[affected_old_ents < base_num_ent]
            if affected_old_ents.numel() > 0
            else torch.empty(0, dtype=torch.long, device=self.device)
        )

        update_mask = torch.zeros(
            self.model.entity_emb.weight.shape[0], dtype=torch.bool, device=self.device
        )
        ...
        with torch.no_grad():
            if affected_mask.any():
                ...
                mu_old_affected, _ = self.model.predict(
                    old_z[affected_edges_snap[0]],
                    affected_rels_snap,
                    old_z[affected_edges_snap[1]],
                )
        ...
        for _ in range(steps):
            ...
            if affected_mask.any():
                ...
                mu_pred_old, _ = self.model.predict(
                    curr_z[affected_edges[0]], affected_rels, curr_z[affected_edges[1]]
                )
            ...
            if affected_mask.any() and mu_old_affected is not None:
                ...
                mu_cur_affected, _ = self.model.predict(
                    curr_z[affected_edges_cur[0]],
                    affected_rels_cur,
                    curr_z[affected_edges_cur[1]],
                )
    ...
    def _update_dataset_belief(self, base_edge_idx, base_edge_type, updated_beliefs):
        """将贝叶斯滤波后的置信度写回数据集的全局 belief state。"""
        h_list = base_edge_idx[0].cpu().numpy()
        t_list = base_edge_idx[1].cpu().numpy()
        r_list = base_edge_type.cpu().numpy()
